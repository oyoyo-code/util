...
REM PowerShellを使用してファイルの日付チェックと削除を実行（改行版）
powershell -ExecutionPolicy Bypass -Command "^
$cutoffDate = [DateTime]'%CUTOFF_DATE%'; ^
Write-Host '対象パス: %TARGET_PATH%'; ^
Write-Host '拡張子: *.%FILE_EXTENSION%'; ^
Write-Host ('カットオフ日: ' + $cutoffDate.ToString('yyyy-MM-dd')); ^
Write-Host 'サブフォルダ含む: %INCLUDE_SUBDIRS%'; ^
Write-Host ''; ^
if ('%INCLUDE_SUBDIRS%' -eq 'Y') { ^
    $files = Get-ChildItem -Path '%TARGET_PATH%' -Filter '*.%FILE_EXTENSION%' -Recurse; ^
} else { ^
    $files = Get-ChildItem -Path '%TARGET_PATH%' -Filter '*.%FILE_EXTENSION%'; ^
} ^
